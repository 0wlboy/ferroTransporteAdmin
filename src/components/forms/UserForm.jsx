...
export default function UserForm({
    ...
    if (initialData) {
      ...
      setFullName(
        initialData.fullName ||
          (initialData.primer_nombre
            ? `${initialData.primer_nombre} ${initialData.apellido || ""}`.trim()
            : "") ||
          "",
      );
      ...
      setRole(initialData.role || initialData.rol || "");
    }
  }, [initialData]);
  ...
  const validateFullName = (val) => {
    ...
    if (trimmed.split(/\s+/).length < 2)
      return "Ingrese el nombre completo (mínimo nombre y apellido).";
    if (!nameRegex.test(trimmed))
      return "El nombre debe contener entre 4 y 60 caracteres alfabéticos.";
    return "";
  };
  ...
  const validateTelefono = (val) => {
    ...
    if (!/^\d{10,15}$/.test(stripped))
      return "El teléfono debe contener entre 10 y 15 dígitos numéricos.";
    return "";
  };
  ...
  const isFormFilled =
    isEdit || (email && password && fullName && ci && telefono && idGerencia);
  const isDirty =
    !isEdit ||
  ...
  const isSubmitDisabled =
    !isFormFilled || !isDirty || hasAnyError || isLoading;
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          <Input
            ...
            placeholder={
              isEdit ? "Escriba una nueva contraseña (opcional)" : "Contraseña"
            }
            icon={Lock}
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              setErrors((prev) => ({
                ...prev,
                ci: validateCi(e.target.value),
              }));
            }}
            onBlur={() =>
              setErrors((prev) => ({ ...prev, ci: validateCi(ci) }))
            }
            error={errors.ci}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            ...
            onChange={(e) => {
              ...
              setErrors((prev) => ({
                ...prev,
                telefono: validateTelefono(e.target.value),
              }));
            }}
            onBlur={() =>
              setErrors((prev) => ({
          ...
          <Select
            ...
            onChange={(val) => {
              ...
              setErrors((prev) => ({
                ...prev,
                idGerencia: validateGerencia(val),
              }));
            }}
            onBlur={() =>
              setErrors((prev) => ({
                ...prev,
                idGerencia: validateGerencia(idGerencia),
              }))
            }
