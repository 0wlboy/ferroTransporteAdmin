...
export default function Home() {
    ...
    const { stats, chartData, urgentPetitions, recentPetitions, loading } = useHomeStats();

    const firstName = currentUser?.name ? currentUser.name.split(" ")[0] : "Javier";

    // Headers for Urgent Petitions (Sidebar)
    const urgentHeaders = [
        {
            label: "PASAJERO",
            headerClassName: "text-white/80 font-bold text-[10px] tracking-wider py-3 px-4",
    ...
    const recentHeaders = [
        ...
        {
            label: "CONDUCTOR",
            headerClassName: "text-[10px] font-black text-[#8A1538] tracking-wider py-4 px-4",
        ...
        },
        {
            ...
            render: (item) => {
                ...
                const badgeClass = status === "Completado"
                ...
                return (
                    <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-[10px] font-bold tracking-wide ${badgeClass}`}>
                        {status}
                    </span>
                );
            }
    ...
    const formattedTotalCount = stats.solicitudesTotalHoy >= 1000
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl border border-[#F3E8EB] shadow-xs p-6 flex flex-col justify-between select-none">
                    ...
                    <div className="flex-1 min-h-[260px] flex items-center justify-center relative">
                        ...
                        <ResponsiveContainer width="100%" height={260}>
                            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F3E8EB" />
                                <XAxis
                                    dataKey="name"
                                    axisLine={false}
                                    tickLine={false}
                                ...
                                <Area
                                    ...
                                    dot={{ r: 4, stroke: "#8A1538", strokeWidth: 2, fill: "#fff" }}
                                    activeDot={{ r: 6, stroke: "#8A1538", strokeWidth: 2, fill: "#8A1538" }}
                                />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-4 px-2 text-xs">
                        ...
                        <span className="text-gray-400 font-semibold tracking-wide">Peticiones</span>
                        <span className="text-gray-800 font-black tracking-wide pl-1">{formattedTotalCount} total</span>
                    </div>
                </div>
