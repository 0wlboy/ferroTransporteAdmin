    ...
    const fetchVehicles = async () => {
      ...
      try {
        ...
        if (searchTerm.trim() !== "") {
          ...
          query = query.or(
            `placa.ilike.${term},marca.ilike.${term},modelo.ilike.${term},ci_driver.ilike.${term}`,
          );
        }
        ...
        else if (sortBy === "placa_asc")
          query = query.order("placa", { ascending: true });
        else if (sortBy === "placa_desc")
          query = query.order("placa", { ascending: false });
        else if (sortBy === "estado_asc")
          query = query.order("estado", { ascending: true });
        else if (sortBy === "estado_desc")
        ...
        const uniqueCis = [
          ...new Set(vehicles.flatMap((v) => [v.ci_driver]).filter(Boolean)),
        ];
        ...
        if (uniqueCis.length > 0) {
          ...
          users?.forEach((u) => {
            userMap[u.ci_user] =
              `${u.primer_nombre || ""} ${u.apellido || ""}`.trim();
        ...
        const formatted = vehicles.map((v) => ({
          ...
          driverName: v.ci_driver
            ? userMap[v.ci_driver] || v.ci_driver
            : "Por Asignar",
